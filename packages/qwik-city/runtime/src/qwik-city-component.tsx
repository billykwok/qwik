...
import { clientHistory } from './client-navigate';
    ...
    async function run() {
      ...
      if (loadedRoute) {
        ...
        if (isBrowser) {
          ...
          if (isSameOrigin(prevUrl, trackUrl)) {
            const samePath = isSamePath(prevUrl, trackUrl);
            const sameHash = prevUrl.hash === trackUrl.hash;
            if (navType === 'popstate') {
              clientHistory.onPopState();
            } else {
              // push to history for path or hash changes
              if (!samePath || !sameHash) {
                clientHistory.pushState(toPath(trackUrl));
              }
              // mimic native hashchange event
              if (navType === 'link' && samePath && !sameHash) {
                window.dispatchEvent(
                  new HashChangeEvent('hashchange', { newURL: trackUrl.href, oldURL: prevUrl.href })
                );
              }
            }
          }
